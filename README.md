...
- 生产系统：农田真实耕种、商业交易、工业 JSON 工作流、辐射型外部作业、真实机器步骤和物流基础表。
...
- 工业系统新增辐射型作业区 `workArea`，可让工业建筑在结构外矩形范围内工作；内置 `lumberjacks_house` 伐木工小屋使用该机制扫描建筑外圈 32 格内的自然树。
- 新增通用 `harvest_block_clusters` 与 `deposit_carried_items` 工作步骤：NPC 会移动到树桩旁，站定后只显示方块裂纹进度，挖掘完成时由服务端一次性移除整棵树并把真实掉落写入 `workState`，随后回小屋木桶入库。
- 工业控制箱状态已支持工作中视图推送，状态变化会节流同步给附近客户端，便于观察“前往树桩、挖树桩、移除整棵树、输出满”等实时状态。
